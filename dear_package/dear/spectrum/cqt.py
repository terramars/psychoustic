...
class Spectrum(SpectrumBase):
    ...
    def pre_calculate(Q, k_max, win, win_shape):
        ...
        #
        t = 1 + 1/float(Q)
        WL = [max(2, int(round(win / t**k))) for k in range(k_max)]
        var['WL'] = WL
        PRE = []
        for wl in WL:
...
class CNTSpectrum(SpectrumBase):
    ...
    def pre_calculate(N, k_max, win, win_shape, sr=None, resize=True):
        ...
        WL = numpy.array(
            [win/2.**(float(k)/N) for k in range(k_max)])
        if resize:
            if sr is None:
                sr = win
    ...
    def walk(self, N, freq_base=A0, freq_max=A9, hop=0.02, start=0, end=None,
             join_channels=True, win_shape=numpy.hanning, resize_win=False):
        ''''''
        n, k_max, win, step, var = self._calculate_params(N, freq_base,
                                                          freq_max, hop, win_shape, resize_win)
        print(n, k_max, win, step, hop)
        transform = self.transform
        for samples in self.audio.walk(win, step, start, end, join_channels):
            if join_channels:
